  ...
  const ordered = [...registrations]
    ...
    .map((registration): PreparedBankRegistration => {
      ...
      const categories = [...registration.categories]
        .sort((left, right) => compareIds(left.categorySetId, right.categorySetId));
      ...
      for (const category of categories) {
        ...
        const priorCategoryBatch = seenCategorySetIds.get(category.categorySetId);
...
const PREPARED_EASY_EXPANSION_BANKS = prepareBanks(REGISTERED_EASY_EXPANSION_BANKS);
...
export function getEasyExpansionBank(batchId: string): readonly EasyExpansionCategory[] {
  const registration = PREPARED_EASY_EXPANSION_BANKS.find(
    (candidate) => candidate.batchId === batchId,
  );
  if (registration === undefined) {
    throw new Error(`No Easy expansion bank registered for batch "${batchId}".`);
  }
